    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#111726] border border-slate-800 rounded-xl p-4">
          <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">Manifests Audited</p>
          <p className="text-2xl font-bold text-white mt-1">{iacData?.filesAudited || 0}</p>
        ...
        <div className="bg-[#111726] border border-rose-500/30 rounded-xl p-4">
          <p className="text-xs text-rose-400 font-medium uppercase tracking-wider">Critical Misconfigs</p>
          <p className="text-2xl font-bold text-rose-400 mt-1">{iacData?.criticalCount || 0}</p>
        ...
        <div className="bg-[#111726] border border-amber-500/30 rounded-xl p-4">
          <p className="text-xs text-amber-400 font-medium uppercase tracking-wider">High Risk</p>
          <p className="text-2xl font-bold text-amber-400 mt-1">{iacData?.highCount || 0}</p>
          <p className="text-[11px] text-amber-500/80 mt-1">Exposed Database Ports</p>
        </div>

        <div className="bg-[#111726] border border-purple-500/30 rounded-xl p-4">
          <p className="text-xs text-purple-400 font-medium uppercase tracking-wider">IaC Posture Score</p>
          <p className="text-2xl font-bold text-purple-400 mt-1">{iacData?.postureScore || 100}/100</p>
      ...
      <div className="bg-[#111726] border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full bg-[#0a0f1d] border border-slate-800 rounded-lg pl-9 pr-3 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-purple-500"
            />
          </div>
        ...
        {filteredFindings.length === 0 ? (
          <div className="p-12 text-center text-slate-400">
            ...
            <h4 className="text-white font-medium mb-1">Container & Infrastructure Hardened</h4>
            <p className="text-xs text-slate-500">No dangerous privilege escalations or exposed port bindings detected.</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-800/80">
            {filteredFindings.map((finding) => (
              <div key={finding.id} className="p-5 hover:bg-[#151a2d] transition-colors space-y-3">
                ...
                {/* Problematic snippet */}
                {finding.codeSnippet && (
                  <div className="bg-[#0a0f1d] border border-slate-800 rounded-lg p-2.5 font-mono text-xs text-rose-300 truncate">
                    <code>{finding.codeSnippet}</code>
                  </div>
                )}

                {/* Remediation Box */}
                <div className="bg-[#120e24] border border-purple-500/30 rounded-xl p-3 flex items-start gap-2.5 text-xs text-purple-200">
                  <Lock className="w-4 h-4 text-purple-400 mt-0.5 shrink-0" />
                  <div>
                    <span className="font-semibold text-purple-300">Remediation Guidance: </span>
                    {finding.remediation}
                  </div>
                </div>
              </div>
