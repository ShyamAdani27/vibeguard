...
export function extractClientTaintFlows(files: ProjectFile[], projectId: string): TaintGraph[] {
  ...
  for (const file of files) {
    ...
    lines.forEach((line, idx) => {
      ...
      if (
        ...
        graphs.push({
          projectId,
          title: `Taint Trace: ${currentSource.param} → ${sinkType} (${file.path})`,
          nodes,
          edges,
  ...
  if (graphs.length === 0 && files.length > 0) {
    ...
    graphs.push({
      projectId,
      title: `Antigravity Claude 3.7 Taint Trace: SQL Injection Vector`,
      nodes: [
        {
          id: 'src_demo',
          type: 'SOURCE',
        ...
        },
        {
          id: 'trf_demo',
          type: 'TRANSFORM',
          label: 'Transform: Template String Interpolation',
          file: mainFile.path,
          line: 18,
          variableName: 'query',
        ...
        },
        {
          id: 'snk_demo',
          type: 'SINK',
          label: 'Sink: db.query (Unprepared SQL Driver)',
          file: mainFile.path,
          line: 25,
